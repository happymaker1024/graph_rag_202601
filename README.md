...
### 특장점:
- 정확한 관계 기반 검색: 그래프의 노드-관계 구조를 활용해 다단계 연결, 경로, 패턴 등 복잡한 관계 질의를 정확하게 수행할 수 있음.
- 컨텍스트 유지: 엔티티 간의 관계가 명시적으로 저장되어 있어 연결된 의미 단위의 컨텍스트를 유지하여 더 풍부한 컨텍스트를 제공함.
- 구조화된 정보 검색: 단순 텍스트 검색이 아닌 스키마와 관계를 기반으로 한 구조화된 방식으로 정보 검색이 가능함.
